...
async def test_register_user(client: AsyncClient, async_session: AsyncSession) -> None:
    response = await client.post('/signin',
                                 json={
                                     'name': test_user.name,
...
async def test_get_token(client: AsyncClient, async_session: AsyncSession) -> None:
...
async def test_get_user_info(client: AsyncClient, async_session: AsyncSession) -> None:
    response = await client.get('/me', headers={'Authorization': f'Bearer {test_user.token}'})
...
async def test_get_availability_services(client: AsyncClient, async_session: AsyncSession) -> None:
...
async def test_upload_file(client: AsyncClient, async_session: AsyncSession) -> None:
...
async def test_get_file_list(client: AsyncClient, async_session: AsyncSession) -> None:
    response = await client.get('/list', headers={'Authorization': f'Bearer {test_user.token}'})
    assert response.status_code == status.HTTP_200_OK
